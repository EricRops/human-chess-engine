...
aws ec2 run-instances \
	...
	--ebs-optimized \
	--block-device-mapping "[ { \"DeviceName\": \"/dev/sda1\", \"Ebs\": { \"VolumeSize\": 10000 } } ]"
...
sed -i "/^\([[:space:]]*cluster_name: \).*/s//\1${clust_name}/" ./conf/cassandra.yaml
sed -i "/^\([[:space:]]*listen_address: \).*/s//\1${priv_ip}/" ./conf/cassandra.yaml
sed -i "/^\([[:space:]]*rpc_address: \).*/s//\1${priv_ip}/" ./conf/cassandra.yaml
sed -i "/^\([[:space:]]*endpoint_snitch: \).*/s//\1${endpoint_snitch}/" ./conf/cassandra.yaml
# Seed nodes - Fow now, have just one seed node (the "master")
...
scp ./conf/cassandra.yaml worker1:apache-cassandra-3.11.8/conf/cassandra.yaml
scp ./conf/cassandra.yaml worker2:apache-cassandra-3.11.8/conf/cassandra.yaml
...
scp ./conf/cassandra-rackdc.properties worker1:apache-cassandra-3.11.8/conf/cassandra-rackdc.properties
scp ./conf/cassandra-rackdc.properties worker2:apache-cassandra-3.11.8/conf/cassandra-rackdc.properties
